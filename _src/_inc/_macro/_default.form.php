<?php
	...
	if( isset( $_REQUEST[ $tb ]['id'] ) && ! empty( $_REQUEST[ $tb ]['id'] ) ) {
	    ...
	    $ct['page']['parents']['h1'][ max( array_keys( $ct['page']['parents']['h1'] ) ) ][ LINGUA_CORRENTE ] = mysqlSelectValue( $cf['mysql']['connection'], 'SELECT __label__ FROM ' . $tb . '_view WHERE id = ?', array( array( 's' => $_REQUEST[ $tb ]['id'] ) ) );
