...
chiave                  | descrizione
------------------------|---------------------------------------------------------------------------------------
view.table              | il nome dell'entità su cui lavorare, di solito una tabella del database (ma potrebbe anche trattarsi di una view virtuale, vedi la documentazione relativa al database per ulteriori dettagli)
...
view.open.table         | il nome della tabella del database da utilizzare per aprire la riga; questo è necessario soprattutto quando la view fa riferimento a un'entità virtuale (vedi la documentazione relativa al database)
...
$cf['request']['__view__'] nella documentazione relativa alle variabili del framework.
...
[...] per ordinare nativamente le view si può utilizzare l'array $ct['view']['__sort__'] ad esempio [...] che viene poi integrato con
$_REQUEST['__view__'][ $ct['view']['id'] ]['__sort__'] in _src/_inc/_macro/_default.view.php [...]
...
di inserimento e aggiornamento delle righe (vedi la documentazione relativa al database per ulteriori
...
<fieldset id="{{ subtable ~ legend|replace({' ': '_', '\'': '_' }) }}">
    {{ frm.subformLegend( subtable ~ legend|replace({' ': '_', '\'': '_' }), legend ) }}
...
Inoltre è opportuno notare che, nel codice Javascript con cui si richiama il webservice di attivazione dei task, l'utilizzo della notazione
sintetica per l'URL anziché di quella esplicita consente di sfruttare la capacità del file .htaccess di attivare il task custom se esiste al
posto di quello standard; se invece si utilizzasse l'URL esplicito, verrebbe sempre chiamato quello senza possibilità di scelta; per
comprendere appieno la differenza si confronti il seguente codice:
```js
function importa_anagrafiche() {
  var data = $('#__upload_____anagrafica__').val();
...
```
Nel primo caso verrà sempre e solo utilizzato il task standard (/_src/_api/_task/_anagrafica.importazione.start.php) mentre nel secondo caso verrà
utilizzato il custom se esiste (/src/api/task/anagrafica.importazione.start.php) e solo in caso questo non esista verrà utilizzato il task standard;
per ulteriori approfondimenti su questo meccanismo si veda il file .htaccess.
