	...
	foreach( $ct['view']['data'] as &$row ) {
		if( is_array( $row ) ) {
			if( ! empty( $row['se_pacchetto'] ) ) {
				$idMastro = mysqlSelectCachedValue( $cf['memcache']['connection'], $cf['mysql']['connection'], 'SELECT mastri.id FROM mastri INNER JOIN tipologie_mastri ON tipologie_mastri.id = mastri.id_tipologia WHERE mastri.id_progetto = ? AND tipologie_mastri.se_registro IS NOT NULL', array( array( 's' => $row['id_progetto'] ) ) );
			...
			$row['id_mastro_provenienza'] = mysqlSelectCachedValue( $cf['memcache']['connection'], $cf['mysql']['connection'], 'SELECT __report_giacenza_ore__.id FROM __report_giacenza_ore__ WHERE id_progetto = ?', array( array( 's' => $row['id_progetto'] ) ) );
            $onclick = "$('#attivita_id_progetto').val('".$row['id_progetto']."');$('#attivita_id_todo').val('".$row['id']."');".
                $mastro."$('#attivita_id_mastro_provenienza').val('".$row['id_mastro_provenienza']."');".
                "$('#scorciatoia_attivita').modal('show');";
			$row[ NULL ] = '<a href="#" data-toggle="modal" data-target="#scorciatoia_attivita" onclick="'.$onclick.'"><i class="fa fa-pencil-square-o"></i></a>'.
				'<a href="' . $cf['page']['path'][ LINGUA_CORRENTE ] . '?__unsprint__=' . $row['id'] . '"><i class="fa fa-arrow-left"></i></a>';
		}
	}

	timerCheck( $cf['speed'], 'fine elaborazione dati vista' );
