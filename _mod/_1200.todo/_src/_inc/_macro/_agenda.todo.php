<?php
	...
	if( isset( $_SESSION['account']['id_anagrafica'] ) ){
		...
		$ct['etc']['todo_da_fare'] = mysqlQuery(
			$cf['mysql']['connection'],
			'SELECT todo_view.* FROM todo_view LEFT JOIN attivita ON attivita.id_todo = todo_view.id WHERE attivita.data_attivita IS NULL AND attivita.id_anagrafica = ? GROUP BY todo_view.id',
		...
		$ct['etc']['todo_da_fare_per_altri'] = mysqlQuery(
			$cf['mysql']['connection'],
			'SELECT todo_view.* FROM todo_view LEFT JOIN attivita ON attivita.id_todo = todo_view.id WHERE attivita.data_attivita IS NULL AND attivita.id_anagrafica <> ? AND todo_view.id_responsabile <> ? GROUP BY todo_view.id',
		...
		$ct['etc']['todo_responsabile'] = mysqlQuery(
			$cf['mysql']['connection'],
			'SELECT todo_view.*, count(attivita.id) AS n_attivita FROM todo_view LEFT JOIN attivita ON attivita.id_todo = todo_view.id WHERE attivita.data_attivita IS NULL AND attivita.id_anagrafica <> ? AND todo_view.id_responsabile = ? AND todo_view.timestamp_completamento IS NULL GROUP BY todo_view.id HAVING n_attivita > 0',
			array( array( 's' => $_SESSION['account']['id_anagrafica'] ), array( 's' => $_SESSION['account']['id_anagrafica'] ) )
		);
		...
		$ct['etc']['todo_da_pianificare'] = mysqlQuery(
			$cf['mysql']['connection'],
			'SELECT todo_view.*, count(attivita.id) AS n_attivita FROM todo_view LEFT JOIN attivita ON attivita.id_todo = todo_view.id WHERE attivita.data_attivita IS NULL AND todo_view.id_responsabile = ? AND todo_view.timestamp_completamento IS NULL GROUP BY todo_view.id HAVING n_attivita = 0',
			array( array( 's' => $_SESSION['account']['id_anagrafica'] ) )
		);
//print_r($ct['etc']['todo_da_pianificare']);
		...
		$ct['etc']['todo_da_pianificare'] = mysqlQuery(
			$cf['mysql']['connection'],
			'SELECT todo_view.*, count(attivita.id) AS n_attivita FROM todo_view LEFT JOIN attivita ON attivita.id_todo = todo_view.id WHERE attivita.data_attivita IS NULL AND todo_view.timestamp_completamento IS NULL GROUP BY todo_view.id HAVING n_attivita = 0'
		);
