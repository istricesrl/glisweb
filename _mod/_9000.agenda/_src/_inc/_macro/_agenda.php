<?php
	...
	if( isset( $_SESSION['account']['id_anagrafica'] ) ){
		...
		$ct['etc']['todo'] = mysqlQuery(
			...
			'WHERE attivita.data_attivita IS NULL AND (todo.id_anagrafica = ? AND attivita.id_anagrafica_programmazione <> ?) AND todo.data_chiusura IS NULL'.( isset($todo) ? ' AND todo.id NOT IN ('.implode(',',$todo).')' : '' ).'GROUP BY todo.id  HAVING n_attivita > 0',
