<?php
    ...
    if( !empty( $jo ) ){
        foreach( $jo as $j ){
            ...
            $ct['etc']['report_operatori'][ $j['id'] ] = $j;
            $ct['etc']['report_operatori'][ $j['id'] ]['nome'] = int2month($wksp['mese']) . ' ' . $wksp['anno'];
            
            if( !empty( $j['timestamp_completamento'] ) ){
                $ct['etc']['report_operatori'][ $j['id'] ]['stato'] = 'completato';
            }
            elseif( $j['corrente'] < $j['totale'] ){
                $ct['etc']['report_operatori'][ $j['id'] ]['stato'] = 'in corso';
            }
    ...
    if( !empty( $jc ) ){
        foreach( $jc as $j ){
            ...
            $ct['etc']['report_clienti'][ $j['id'] ] = $j;
            $ct['etc']['report_operatori'][ $j['id'] ]['nome'] = int2month($wksp['mese']) . ' ' . $wksp['anno'];
            
            if( !empty( $j['timestamp_completamento'] ) ){
                $ct['etc']['report_clienti'][ $j['id'] ]['stato'] = 'completato';
            }
            elseif( $j['corrente'] < $j['totale'] ){
                $ct['etc']['report_clienti'][ $j['id'] ]['stato'] = 'in corso';
            }
    ...
    if( !empty( $joc ) ){
        foreach( $joc as $j ){
            ...
            $ct['etc']['report_operatori_per_cliente'][ $j['id'] ] = $j;
            $ct['etc']['report_operatori'][ $j['id'] ]['nome'] = int2month($wksp['mese']) . ' ' . $wksp['anno'];
            
            if( !empty( $j['timestamp_completamento'] ) ){
                $ct['etc']['report_operatori_per_cliente'][ $j['id'] ]['stato'] = 'completato';
            }
            elseif( $j['corrente'] < $j['totale'] ){
                $ct['etc']['report_operatori_per_cliente'][ $j['id'] ]['stato'] = 'in corso';
            }
