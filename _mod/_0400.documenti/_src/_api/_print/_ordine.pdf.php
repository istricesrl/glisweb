<?php
    ...
    $doc['righe'] = mysqlQuery(
        $cf['mysql']['connection'],
        'SELECT documenti_articoli_view.* FROM documenti_articoli_view '.
	...
	$dobj = 'ordine n. ' . $doc['numero'] . ' del ' . strftime( '%d %B %Y', strtotime( $doc['data'] ) );
	...
	$pdf->Cell( $col * 2, 0, 'q.tà', $brdh, 0, 'L' );				// larghezza, altezza, testo, bordo, newline, allineamento
	$pdf->Cell( $col * 2, 0, 'udm', $brdh, 1, 'L' );				// larghezza, altezza, testo, bordo, newline, allineamento
...
//	$pdf->Cell( $col * 3, 0, 'magazzino carico', $brdh, 1, 'C' );				// larghezza, altezza, testo, bordo, newline, allineamento
	...
	foreach( $doc['righe'] as $row ) {
	    $trh = $pdf->GetStringHeight( $col * 8,$row['prodotto'] , false, true, '', 'B' );				// 
	$pdf->SetFont( $fnt, '', $fnts );
	    ...
	    $pdf->MultiCell( $col * 8, $lh,$row['prodotto'], $brdc, 'L', 1, 0 );					// w, h, testo, bordo, allineamento, riempimento, newline

	    // scrivo in grassetto le righe che sono aggregazioni di righe

//	    if( $row['nome'][0] === '*' ){$pdf->SetFillColor(255, 0, 0);} 
	    $pdf->Cell( $col * 2, $trh, $row['quantita'], $brdc, 0, 'L', 1, '', 0, 1, 'T', 'T' );	// larghezza, altezza, testo, bordo, newline, allineamento
	    $pdf->Cell( $col * 2, $trh, $row['udm'], $brdc, 1, 'C', 1, '', 0, false, 'T', 'T' );			// larghezza, altezza, testo, bordo, newline, allineamento
