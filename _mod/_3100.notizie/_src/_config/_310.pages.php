    ...
    if ($cf['contents']['cached'] === false) {
            ...
            foreach ($pgs as $pg) {
                ...
                if ($pg['timestamp_aggiornamento'] > $age || empty($pgc)) {
                    ...
                    if (empty($pg['template'])) {
                        $pg['template'] = $cf['notizie']['pages']['elenco']['template'];
                    }
                    if (empty($pg['schema_html'])) {
                        $pg['schema_html'] = $cf['notizie']['pages']['elenco']['schema'];
                    }
                    if (empty($pg['tema_css'])) {
                        $pg['tema_css'] = $cf['notizie']['pages']['elenco']['css'];
                    }
        ...
        $pgs = mysqlQuery(
            ...
            'SELECT notizie.*, notizie_categorie.id_categoria AS id_categoria, 
                ...
                WHERE ( categorie_notizie.id_sito = ? OR categorie_notizie.id_sito IS NULL )
                AND ( pubblicazioni.timestamp_inizio IS NULL OR pubblicazioni.timestamp_inizio < ? ) 
                AND ( pubblicazioni.timestamp_fine IS NULL OR pubblicazioni.timestamp_fine > ? ) 
                GROUP BY notizie.id',
            array(
            ...
            foreach ($pgs as $pg) {
                ...
                foreach ($cat as $ce) {
                    ...
                    if ($pg['timestamp_aggiornamento'] > $age || empty($pgc)) {
                        ...
                        if (empty($pg['template'])) {
                            $pg['template'] = $cf['notizie']['pages']['scheda']['template'];
                        }
                        if (empty($pg['schema_html'])) {
                            $pg['schema_html'] = $cf['notizie']['pages']['scheda']['schema'];
                        }
                        if (empty($pg['tema_css'])) {
                            $pg['tema_css'] = $cf['notizie']['pages']['scheda']['css'];
                        }
