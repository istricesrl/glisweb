<?php
    ...
    /**
     * salvataggio in cache dei dati delle notizie
     *
     *
     *
     *
    ...
    if( ! empty( $cf['memcache']['connection'] ) ) {
        ...
        if( $cf['contents']['cached'] === false ) {
            ...
            memcacheWrite( $cf['memcache']['connection'], NOTIZIE_PAGES_UPDATED, $cf['notizie']['updated'] );
            memcacheWrite( $cf['memcache']['connection'], NOTIZIE_INDEX_KEY, $cf['notizie']['index'] );
            memcacheWrite( $cf['memcache']['connection'], NOTIZIE_PAGES_CACHED, time() );

            // timer
            timerCheck( $cf['speed'], '-> fine scrittura cache pagine' );

            // log
            logger( 'struttura delle pagine scritta in cache', 'speed', LOG_INFO );
