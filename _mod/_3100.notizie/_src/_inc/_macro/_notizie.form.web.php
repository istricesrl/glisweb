<?php
    ...
    if( !isset( $_REQUEST[ $ct['form']['table'] ]['id'] ) && isset( $_REQUEST[ '__preset__' ] ) ) {
	    ...
	    $ct['etc']['caratteristiche_categoria'] = mySqlQuery( $cf['mysql']['connection'], 'SELECT * FROM categorie_notizie_caratteristiche_view WHERE id_categoria = ?', array( array('s' => $_REQUEST[ '__preset__' ]['notizie']['notizie_categorie'][0]['id_categoria'] )) );

	    foreach ( $ct['etc']['caratteristiche_categoria'] as $caratteristiche){
	        $_REQUEST[ $ct['form']['table'] ]['notizie_caratteristiche'][] = array(
