<?php
    ...
    if( isset( $_REQUEST['pagine']['template'] ) ) {
        ...
        } elseif( file_exists( DIR_BASE . $_REQUEST['pagine']['template'] . '/etc/template.yaml' ) ) {

            // configurazione del template
            $yaml = new \Symfony\Component\Yaml\Parser();
            $config = $yaml->parse( file_get_contents( DIR_BASE . $_REQUEST['pagine']['template'] . '/etc/template.yaml' ) );
