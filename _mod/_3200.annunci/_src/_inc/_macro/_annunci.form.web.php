<?php
    ...
    if( !isset( $_REQUEST[ $ct['form']['table'] ]['id'] ) && isset( $_REQUEST[ '__preset__' ] ) ) {
	    ...
	    $ct['etc']['caratteristiche_categoria'] = mySqlQuery( $cf['mysql']['connection'], 'SELECT * FROM categorie_annunci_caratteristiche_view WHERE id_categoria = ?', array( array('s' => $_REQUEST[ '__preset__' ]['annunci']['annunci_categorie'][0]['id_categoria'] )) );

	    foreach ( $ct['etc']['caratteristiche_categoria'] as $caratteristiche){
	        $_REQUEST[ $ct['form']['table'] ]['annunci_caratteristiche'][] = array(
...
*/
	...
	$ct['etc']['select']['tipologie'] = mysqlCachedIndexedQuery(
	    ...
	    $cf['memcache']['connection'],
	    $cf['mysql']['connection'], 
        'SELECT id, __label__ FROM tipologie_annunci_view' 
    );
