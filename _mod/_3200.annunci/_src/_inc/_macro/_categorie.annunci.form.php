<?php
	...
	$ct['etc']['select']['categorie'] = mysqlCachedIndexedQuery(
	    ...
	    $cf['memcache']['connection'],
	    $cf['mysql']['connection'],
	    'SELECT id, __label__ FROM categorie_annunci_view'
	);
	...
	if( isset( $_REQUEST[ $ct['form']['table'] ]['id_sito'] ) ) {
		...
		$ct['etc']['select'][ $ct['form']['table'] ] = mysqlCachedIndexedQuery(
            ...
            $cf['memcache']['connection'],
                $cf['mysql']['connection'],
            'SELECT id, __label__ FROM categorie_annunci_view WHERE id_sito = ? AND categorie_annunci_path_check( categorie_annunci_view.id, ? ) = 0',
            array(
