<?php
    ...
    if( isset( $_REQUEST['categorie_prodotti']['template'] ) ) {
        ...
        } elseif( file_exists( DIR_BASE . $_REQUEST['categorie_prodotti']['template'] . '/etc/template.yaml' ) ) {

            // configurazione del template
            $yaml = new \Symfony\Component\Yaml\Parser();
            $config = $yaml->parse( file_get_contents( DIR_BASE . $_REQUEST['categorie_prodotti']['template'] . '/etc/template.yaml' ) );
        } else {
        ...
        foreach( glob( DIR_BASE . glob2custom( $_REQUEST['categorie_prodotti']['template'] ) . 'etc/template.add.conf', GLOB_BRACE ) as $addCnf ) {
