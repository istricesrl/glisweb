<?php
	...
	$ct['view']['open']['preset']['field'] = 'id_asset';
    ...
    if( isset($_REQUEST[ $ct['form']['table'] ]['id']) ){
	    ...
	    $ct['view']['__restrict__']['id_asset']['EQ'] = $_REQUEST[ $ct['form']['table'] ]['id'];
    }

    $ct['etc']['include']['insert'][] = array(
        'name' => 'insert',
        'file' => 'inc/macchine.form.attivita.insert.html',
        'fa' => 'fa-plus-circle'
    );

    $ct['etc']['include']['insert'][] = array(
        'name' => 'insert_memo',
        'file' => 'inc/macchine.form.attivita.insert.promemoria.html',
    ...
    if( !empty( $ct['view']['data'] ) ) {
		foreach ( $ct['view']['data'] as &$row ){
            ...
            if( !empty( $row['ora_inizio'] ) || !empty( $row['ora_fine'] ) ){ 
                $row['data_attivita'] = 
                    ( ( ! empty( $row['data_attivita'] ) ) ? $row['data_attivita'] : $row['data_fine'] )
                    .'  '
                    .( empty($row['ora_inizio']) ? ' '  : date('H:i', strtotime($row['ora_inizio'])))
                    .' &mdash; '
                    .( empty($row['ora_fine']) ? ' ' :date('H:i', strtotime($row['ora_fine']) ))
                ;
             }
