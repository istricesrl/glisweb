...
CREATE OR REPLACE VIEW colli_view AS
	...
	FROM colli
		...
		LEFT JOIN anagrafica AS a2 ON a2.id = colli.id_anagrafica
	GROUP BY colli.id
	;
...
CREATE OR REPLACE VIEW `corsi_view` AS
	SELECT
		...
		concat( '|', group_concat( DISTINCT f.id SEPARATOR '|' ), '|' ) AS id_fasce,
		group_concat( DISTINCT if( f.id, categorie_progetti_path( f.id ), null ) SEPARATOR ' | ' ) AS fasce,
		concat( '|', group_concat( DISTINCT d.id SEPARATOR '|' ), '|' ) AS id_discipline,
		group_concat( DISTINCT if( d.id, categorie_progetti_path( d.id ), null ) SEPARATOR ' | ' ) AS discipline,
		concat( '|', group_concat( DISTINCT l.id SEPARATOR '|' ), '|' ) AS id_livelli,
...
-- righe_ordini_attivi_view
-- tipologia: vista virtuale
DROP TABLE IF EXISTS `righe_ordini_attivi_view`;
...
-- tipologia: vista virtuale
-- verifica: 2021-10-09 16:02 Fabio Mosti
CREATE OR REPLACE VIEW `righe_ordini_attivi_view` AS
       SELECT
		documenti_articoli.id,
		documenti_articoli.id_genitore,
		documenti_articoli.id_tipologia,
...
-- righe_ordini_passivi_view
-- tipologia: vista virtuale
DROP TABLE IF EXISTS `righe_ordini_passivi_view`;
...
-- tipologia: vista virtuale
-- verifica: 2021-10-09 16:02 Fabio Mosti
CREATE OR REPLACE VIEW `righe_ordini_passivi_view` AS
       SELECT
		documenti_articoli.id,
		documenti_articoli.id_genitore,
		documenti_articoli.id_tipologia,
