CREATE OR REPLACE VIEW `progetti_produzione_view` AS
	...
	if(	( SELECT SUM(ore)
		FROM attivita
		LEFT JOIN task AS t ON t.id = attivita.id_task
		WHERE attivita.id_progetto = progetti.id OR t.id_progetto =progetti.id
		 )  > progetti.ore_previste, 1, 0) AS se_ore_esubero,
	...
	WHERE progetti.timestamp_accettazione IS NOT NULL
