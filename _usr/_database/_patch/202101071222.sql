CREATE OR REPLACE VIEW task_view AS SELECT
    ...
    from_unixtime( task.timestamp_pianificazione, '%d-%m-%Y %H:%i' ) AS data_ora_pianificazione,
    day(from_unixtime( task.timestamp_pianificazione, '%Y-%m-%d %H:%i' )) as giorno,
    month(from_unixtime( task.timestamp_pianificazione, '%Y-%m-%d %H:%i' )) as mese,
    year(from_unixtime( task.timestamp_pianificazione, '%Y-%m-%d %H:%i' )) as anno,
    coalesce(
...
LEFT JOIN luoghi ON luoghi.id = task.id_luogo
