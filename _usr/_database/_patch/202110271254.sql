CREATE OR REPLACE VIEW `ticket_archivio_view` AS SELECT
	todo.id,
	...
	if( todo.timestamp_completamento IS NOT NULL, 2, if( count( attivita.id ), 1, 0 ) ) AS completato,
	concat_ws(
		' | ',
		coalesce(
			anagrafica_cliente.soprannome,
...
WHERE tipologie_attivita.se_ticket = 1 AND todo.timestamp_completamento IS NOT NULL
GROUP BY todo.id
;
