CREATE OR REPLACE VIEW anagrafica_view AS
	...
	max( categorie_anagrafica.se_emittente ) AS se_emittente,
	coalesce(
	...
	coalesce( aRop.soprannome, aRop.denominazione , concat( aRop.cognome, ' ', aRop.nome ), '' ) AS responsabile_operativo,
	...
	group_concat( DISTINCT indirizzi.cap SEPARATOR ' | ' ) as cap
	FROM anagrafica
	...
	LEFT JOIN anagrafica AS aRop ON aRop.id = anagrafica.id_responsabile_operativo
	...
	LEFT JOIN anagrafica_indirizzi ON anagrafica_indirizzi.id_anagrafica = anagrafica.id
