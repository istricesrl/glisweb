CREATE OR REPLACE VIEW `documenti_articoli_view` AS
    ...
    LEFT JOIN mastri AS carico ON carico.id = documenti_articoli.id_mastro_provenienza
    LEFT JOIN mastri AS scarico ON scarico.id = documenti_articoli.id_mastro_destinazione
    ORDER BY __label__
;
