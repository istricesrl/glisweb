CREATE OR REPLACE VIEW `variazioni_attivita_view` AS
	...
	concat( min(periodi_variazioni_attivita.data_inizio), ' > ', max(periodi_variazioni_attivita.data_fine) ) AS periodo_variazione,
	CONCAT( 
		coalesce(
			anagrafica.soprannome,
			anagrafica.denominazione,
	...
	LEFT JOIN periodi_variazioni_attivita ON periodi_variazioni_attivita.id_variazione = variazioni_attivita.id
	GROUP BY variazioni_attivita.id
	ORDER BY __label__
;