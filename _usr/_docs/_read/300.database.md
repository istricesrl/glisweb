...
tabella                         | codice    | tipologia     |  gruppo      | descrizione
--------------------------------|-----------|---------------|--------------|---------------|------------------------------
colori                          | 005100    |               |              | contiene i colori
comuni                          | 005300    |               |               | contiene i comuni
condizioni_pagamento            | 006200    |               |               | contiene le condizioni di pagamento
continenti                      | 007100    |               |               | contiene i continenti
iva                             | 016000    |               |               | contiene le aliquote IVA
lingue                          | 016800    |               |               | contiene le lingue supportate dal framework
modalita_pagamento              | 021900    |               |               | contiene le modalità di pagamento codificate secondo lo standar al 2022-01-18
periodicita                     | 023600    |               |               | contiene le periodicità
provincie                       | 028000    |               |               | contiene le provincie
regimi                          | 029800    |               |               | contiene i regimi fiscali
regioni                         | 030200    |               |               | contiene le regioni
ruoli_anagrafica                | 034000    |               |               | contiene i ruoli svolti dalle anagrafiche
ruoli_articoli                  | 034100    |               |               | contiene i ruoli svolti dagli articoli
ruoli_audio                     | 034200    |               |               | contiene i ruoli svolti dagli audio
...
tipologie_documenti_articoli    | 052700    | ricorsiva     | tipologie     | contiene le tipologie delle righe dei documenti
...
articoli_caratteristiche        | 001600    | relazione     | principale    | mette in relazione articoli e caratteristiche
...
caratteristiche        | 002800    | base          | secondaria    | contiene le caratteristica di indirizzi, edifici e immobili
caratteristiche        | 002900    | base          | principale    | contiene le caratteristiche dei prodotti
...
mailing_liste                   | 019050    | relazione     | secondarie    | mette in relazione mailing e liste
mailing_mail                    | 019100    | relazione     | secondarie    | mette in relazione mailing e mail
...
prodotti_caratteristiche        | 026200    | relazione     | principale    | mette in relazione prodotti e caratteristiche
...
PRODOTTI    | 4100      | articoli, articoli_caratteristiche, audio, caratteristiche, categorie_prodotti, contenuti, file, immagini, listini, listini_clienti, marchi, matricole, menu, metadati, prezzi, prodotti, prodotti_caratteristiche, prodotti_categorie, pubblicazioni, ruoli_articoli, ruoli_audio, ruoli_file, ruoli_immagini, ruoli_prodotti, ruoli_video, tipologie_prodotti, tipologie_pubblicazioni, video
...
IMMOBILIARE | V300      | caratteristiche, edifici, immobili
...
il marcatore che chiude il file
-------------------------------

> **verificato il 2026-09-15 leggendo `_src/_api/_task/_mysql.patch.php`**

Il task accumula le righe di una patch e la esegue **quando incontra il marcatore successivo**.
Non c'è nessuna esecuzione finale a fine file: quello che sta dopo l'ultimo `-- |` viene letto,
accumulato e buttato via senza che nessuno se ne accorga — il task non segnala niente, e la patch
risulta semplicemente non applicata.

Da qui due regole:

- **ogni file di patch finisce con un marcatore**, anche se dopo non c'è niente. La convenzione è
  `-- | FINE` (o `-- | FINE FILE`, come in `_060000999999.constraints.sql`);
- **un file senza nessun marcatore non viene eseguito per niente.** Non è un caso di scuola:
  `_202607221300.sedi.inline.backfill.sql` è stato scritto senza marcatori e al 2026-09-15 non
  risulta eseguito su nessun deploy — la tabella `__patch__` di utensilerialughese non ha nessuna
  patch del 2026.

Vale anche per i file di base, che a oggi non sono tutti a posto: `_010000999999.tables.sql`,
`_030000999999.indexes.sql`, `_080000999999.static.sql` e `_090000999999.views.sql` hanno SQL
dopo l'ultimo marcatore, e quel pezzo non viene eseguito da un'installazione da zero.

il numero della patch
---------------------

Il livello di patch del file si ricava dal nome togliendo i `_` e prendendo le prime dodici cifre,
quindi `_202609151100.qualcosa.sql` vale `202609151100`. I marcatori dentro il file devono essere
maggiori del livello di patch del database e crescenti, perché il task aggiorna il livello a ogni
patch eseguita.

l'idempotenza non è facoltativa
-------------------------------

Una patch gira anche dove la modifica è già stata fatta a mano, e il task si ferma al **primo**
errore lasciando indietro tutte le patch successive. Su MariaDB 10.3 le forme da usare sono
`CREATE TABLE IF NOT EXISTS`, `ADD COLUMN IF NOT EXISTS`, `ADD KEY IF NOT EXISTS`,
`DROP INDEX IF EXISTS`, `INSERT IGNORE`.

Attenzione a `CHANGE COLUMN`, che di idempotente non ha niente: se la rinomina è già avvenuta e il
nome vecchio è stato riusato per un'altra colonna — è il caso di `documenti_articoli.id_tipologia`
il 2026-09-15 — una seconda esecuzione rinomina la colonna sbagliata. In quel caso si aggiunge la
colonna nuova, si travasa il contenuto e si svuota la vecchia con una condizione che riconosca il
travaso appena fatto (`WHERE vecchia = nuova`), così dove il travaso è già stato fatto non si tocca
niente.

Infine: una patch che ricrea una vista la ricrea **per intero**, quindi porta con sé tutte le
dipendenze della versione a monte. Prima di metterla in una patch va verificato che quelle tabelle
esistano davvero sui deploy: la `todo_view` dei file base legge `istruzioni`, che al 2026-09-15 su
utensilerialughese e crmfia non esiste.
...
in che verso scorrono i dati di questo deploy
---------------------------------------------

> **da una segnalazione della sessione che lavora su bernispa, 2026-09-15**

Prima di applicare una patch a un deploy che non si conosce, va fatta una domanda che non si può
indovinare guardando il database: **quale ambiente è la sorgente del dato e quale il ricevente, e
il travaso fra i due è additivo o in upsert?**

Non è una curiosità organizzativa, è quello che decide se un dato mancante sia perso o solo non
ancora arrivato. Su bernispa, il 15/09/2026, 38 sedi risultavano senza comune e con la chiave verso
`indirizzi` che puntava a record inesistenti. Letta dal solo DEV, la conclusione era "il dato non
c'è più" — e da lì il passo successivo sarebbe stato ricostruire 38 indirizzi a mano. Il dato
invece era intatto sulla sorgente: quel DEV riceve da TEST un travaso per tabella che è additivo
(`INSERT IGNORE`), quindi una riga già presente non viene mai aggiornata e una mai arrivata non
viene ripescata. Il recupero era una copia, non un'invenzione.

Da qui due regole d'ordine, e sono diverse fra loro:

- **una patch di SCHEMA segue l'ordine di sempre**, DEV per primo. È lì che deve rompersi se deve
  rompersi: sempre il 15/09/2026, una patch di schema su un DEV ha fermato il prelievo per due ore,
  ed è stato un bene che capitasse lì;
- **una patch di DATI parte da chi è ricostruibile.** Un ambiente che riceve da un altro si può
  sempre riprendere dalla sorgente; la sorgente no. Applicare per prima una patch di dati
  sull'ambiente che è sorgente vuol dire che, se qualcosa va storto, non c'è nessun posto da cui
  tornare indietro.

La risposta per un progetto specifico non sta qui: sta nelle istruzioni di quel progetto, ed è chi
lo segue a scriverla. Qui sta la domanda, perché è quella che serve a chi arriva da fuori.
